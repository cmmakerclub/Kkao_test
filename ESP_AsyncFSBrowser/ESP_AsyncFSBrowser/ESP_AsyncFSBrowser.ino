...
void setup() {
    ...
    for(int read_loop = 0; read_loop<10; read_loop++){
      delay(40);
      digitalWrite(4,LOW);
      delayMicroseconds(2);
      digitalWrite(4,HIGH);
      ...
      long microsec_tmp = pulseIn(5,HIGH);

      if(microsec_tmp > 0){
        average++;
        microsec += microsec_tmp;
        Serial.println(microsec_tmp);
      }
    }

    RAW_BATT = analogRead(A0);

    digitalWrite(15, LOW);

    if(average > 0) cmdistance = microsec / 58.0f / average;
...
void loop() {
    ...
    if (runMode == MODE_WEBSERVER) {
      ...
      // delay(10);
      //
      // Raw[0] = Wire.read(); // receive a byte as character
      // Raw[1] = Wire.read(); // receive a byte as character
      // Raw[2] = Wire.read(); // receive a byte as character
      // Raw[3] = Wire.read(); // receive a byte as character
      //
      //
      //
      ...
      addDataField(message, batt1, batt2, cmdistance);
